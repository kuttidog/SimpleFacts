...
class NumberViewController: UIViewController {
    ...
    let numberModel: NumberViewable
    weak var delegate: GenerateNumberFact?
    
    
    init(numberModel: NumberViewable) {
    ...
    var date: UITextField = {
        var date = UITextField()
        date.placeholder = "enter only number"
    ...
    @objc func generate(sender: UIButton) {
        ...
        guard let number = Int(numberString) else {
            let alert = UIAlertController(title: "Enter a number to generate a fact", message: "", preferredStyle: .alert)
            let alertAction = UIAlertAction(title: "OK", style: .cancel, handler: nil)
            alert.addAction(alertAction)
            self.present(alert, animated: true, completion: nil)
            return
        }
        let appendString = "math"
        
        numberModel.getNumberData(entertedNumber: number, typeOfInput: appendString) { (factModel, serviceError) in
...
protocol GenerateNumberFact: class{
    func generateNumFact()
}
